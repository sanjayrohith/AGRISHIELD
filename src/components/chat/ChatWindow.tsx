...
export function ChatWindow({ location }: { location: any }) {
  ...
  return (
    <Card className="flex flex-col h-[600px] lg:h-[700px] glass-card bg-white/10 backdrop-blur-xl border border-white/20 shadow-2xl">
      {/* Chat Header */}
      <div className="flex items-center justify-between p-4 border-b border-white/20 bg-white/5 backdrop-blur-sm">
        <div className="flex items-center space-x-3">
          <div className="h-10 w-10 rounded-full bg-white/20 backdrop-blur-sm border border-white/30 flex items-center justify-center p-1 shadow-sm">
        ...
        {/* Language Selector */}
        <div className="flex items-center gap-1 p-1 bg-white/20 backdrop-blur-sm border border-white/30 rounded-lg shadow-sm">
            {['English', 'Hindi', 'Tamil'].map(lang => (
                <Button 
                    ...
                    className={clsx(
                        ...
                        {
                            'bg-white/30 text-white shadow-sm border border-white/40': language === lang,
                            'text-white/80 hover:bg-white/20': language !== lang,
                        }
                    )}
      ...
      {/* Messages Area */}
      <CardContent className="flex-1 overflow-y-auto p-4 space-y-4 bg-white/5 backdrop-blur-sm">
        {messages.map((message) => (
          <MessageBubble key={message.id} message={message} />
        ))}
        {isTyping && (
          <div className="flex items-start space-x-3">
             <div className="h-8 w-8 rounded-full bg-white/20 backdrop-blur-sm border border-white/30 flex items-center justify-center p-1 shadow-sm">
              <AgrishieldLogo size={20} />
            </div>
            <div className="p-3 rounded-lg bg-white/20 backdrop-blur-sm border border-white/30 shadow-sm">
